...
def str_to_method(str, sys_module = sys.modules[__name__]):
	return getattr(sys_module, str)
...
class Form:
	def __init__(self):
		self.class1 = Ui_MainWindow()
		self.app, self.MainWindow, self.ui = setup()
		self.class1.setupUi(self.MainWindow)
		self.user = ''
	...
	def update_speech(self):
		while True:
			self.class1.ment_lb.setText("안녕하세요 세미입니다")
			...
			if self.mic_text in ment_json:
				self.ment_method = str_to_method(ment_json[self.mic_text])
				print(self.ment_method())
			"""	
	...
	def image_select(self):
		...
		if weather_list[3] == 0:
			...
			if weather_list[2] == 1:
				...
				self.class1.weather_icon_lb.setPixmap(QPixmap(pixmap))
	...
	def schedule_reset(self):
		...
		self.class1.schedule_tb.setText(scheduler_string)
	
	def reset_init(self):
		global timer_hour, timer_min, weather_list, timer_sec, ment_string
		self.class1.time_lb.setText("  " + timer_hour + ":" + timer_min + ":" + timer_sec)
		self.class1.wetness_lb.setText("  습도  " + str(weather_list[0]) + "%")
		self.class1.temperature_lb.setText("  온도  " + str(weather_list[1]) + "°C")
		self.class1.dust_lb.setText("미세먼지 " + str(weather_list[4])) 
		#self.class1.Hi.setText(ment_string)
